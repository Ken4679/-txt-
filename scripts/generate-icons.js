...
async function generate() {
  ...
  const png128 = path.resolve('public/icon-128.png');
  const png64 = path.resolve('public/icon-64.png');
  const png48 = path.resolve('public/icon-48.png');
  const png32 = path.resolve('public/icon-32.png');
  const png24 = path.resolve('public/icon-24.png');
  ...
  await sharp(png512).resize(128, 128).png().toFile(png128);
  await sharp(png512).resize(64, 64).png().toFile(png64);
  await sharp(png512).resize(48, 48).png().toFile(png48);
  await sharp(png512).resize(32, 32).png().toFile(png32);
  await sharp(png512).resize(24, 24).png().toFile(png24);
  await sharp(png512).resize(16, 16).png().toFile(png16);

  fs.copyFileSync(png256, rootPng);

  // 2. Generate multi-resolution app.ico for Windows .EXE and Taskbar (all standard Windows sizes)
  const icoBuffer = await pngToIco([png256, png128, png64, png48, png32, png24, png16]);
