...
export const ImportPage: React.FC<ImportPageProps> = ({
  ...
  const sampleTemplate = `已根据您的需求重构了业务入口，并新增了安全辅助函数：
...
def run():
    print(format_greeting("ZipToTxt AI Workspace"))
...
def format_greeting(target: str) -> str:
    return f"[SUCCESS] Service connected for: {target}"
\`\`\`
...
\`\`\``;
  ...
  const handleParse = (): ParsedAiFile[] | null => {
    ...
    try {
      ...
      setSelectedFilePaths(new Set(result.files.map(f => f.relativePath)));
      setWarnings(result.warnings);
      onStatusChange(`成功解析 ${result.files.length} 个文件代码块`, 100);
      onShowToast(
        `✅ 识别到 ${result.files.length} 个文件${
          result.autoClosedCount > 0 ? `（已自动闭合 ${result.autoClosedCount} 处截断）` : ''
        }`,
        'success'
      );
      return result.files;
      ...
      onShowToast(err?.message || '未能解析出有效代码块，请检查格式', 'error');
      return null;
    }
  };
  ...
  const handleGeneratePatchZip = async () => {
    ...
    const filesToPack = files.filter(f => selectedFilePaths.has(f.relativePath));
    if (filesToPack.length === 0) {
      onShowToast('请至少勾选一个需要打包的文件', 'error');
      return;
    }

    const sensitiveCount = filesToPack.filter(f => f.isSensitive).length;
    if (sensitiveCount > 0 && !allowSensitive) {
      onShowToast(
        `检测到 ${sensitiveCount} 个受保护敏感文件（如 .git 或 .env）。请勾选“允许打包敏感文件”`,
        'error'
      );
      return;
    }
    ...
    try {
      const zipBlob = await generatePatchZip(files, allowSensitive, selectedFilePaths);
      ...
      setCopyFeedback('download_patch_zip');
      onStatusChange(`补丁 ZIP 打包完成 · 共 ${filesToPack.length} 个文件`, 100);
      onShowToast(`🎉 已成功生成并下载 ai_patch.zip（包含 ${filesToPack.length} 个修改文件）`, 'success');
    } catch (err: any) {
      setIsGeneratingZip(false);
  ...
  const handleDownloadSingleFile = (file: ParsedAiFile) => {
    ...
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    onShowToast(`已保存并下载单文件 ${filename}`, 'success');
  };

  const handleCopyFileContent = async (file: ParsedAiFile, key: string) => {
    try {
      await navigator.clipboard.writeText(file.content);
      setCopyFeedback(key);
      onShowToast(`已复制 ${file.relativePath} 代码`, 'success');
    } catch {
  ...
  const handleSaveEditedFile = () => {
    ...
    onShowToast(`已保存对 ${previewingFile.relativePath} 的修改`, 'success');
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-slate-900 tracking-tight">02 · 应用 AI 修改 (AI 输出 → 补丁 ZIP)</h2>
        <p className="text-sm text-slate-500 mt-1">
          粘贴大模型返回的 Markdown 代码块（支持各类 ### FILE、**File:**、注释标记），自动提取代码并打包为纯净 patch ZIP。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl border border-slate-200 shadow-xs flex flex-col h-[560px] overflow-hidden">
            ...
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-2.5 py-1.5 text-xs font-medium rounded-lg bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 transition-colors flex items-center gap-1 cursor-pointer shadow-2xs"
                >
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 text-xs font-medium rounded-lg bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-200 text-rose-600 transition-colors flex items-center gap-1 cursor-pointer shadow-2xs"
                >
            ...
            <div className="flex-1 relative bg-white p-2">
              <textarea
                ...
                className="w-full h-full bg-transparent text-slate-800 font-mono text-xs p-3 focus:outline-hidden resize-none leading-relaxed selection:bg-indigo-100"
                spellCheck={false}
              />
            </div>
          ...
          {warnings.length > 0 && (
            <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-xs space-y-1.5">
              ...
              <ul className="list-disc list-inside space-y-0.5 text-amber-800 font-mono text-xs">
                {warnings.map((w, idx) => (
                  <li key={idx}>{w}</li>
                ))}
              </ul>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-xs space-y-4">
            ...
            <label className="flex items-start gap-2.5 p-3 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100/70 cursor-pointer transition-colors">
              <input
                type="checkbox"
            ...
            {sensitiveFiles.length > 0 && (
              <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-900 text-xs space-y-1">
                <div className="flex items-center gap-1.5 font-semibold text-rose-800">
                  ...
                  <span>识别到 {sensitiveFiles.length} 个敏感文件</span>
                </div>
                <ul className="list-disc list-inside text-[11px] text-rose-800 font-mono">
            ...
            <div className="space-y-2 pt-2">
              <button
                ...
                onClick={() => handleParse()}
                className="w-full py-2.5 px-4 rounded-xl text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-800 flex items-center justify-center gap-2 transition-colors cursor-pointer"
              >
              ...
              >
                ...
                {isGeneratingZip ? '正在打包中...' : '生成并下载补丁 patch.zip'}
              </button>
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-emerald-200 shadow-xs space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={selectedFilePaths.size === parsedFiles.length && parsedFiles.length > 0}
                  ...
                  <span className="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                    ...
                    已识别 {parsedFiles.length} 个文件 ({selectedFilePaths.size} 已选)
                  </span>
                </div>
                <span className="text-[11px] text-slate-500">可编辑 / 单独复制</span>
              </div>
              ...
              {parsedFiles.length > 3 && (
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="搜索解析出的文件..."
                    className="w-full pl-8 pr-3 py-1.5 text-xs rounded-lg border border-slate-200 bg-slate-50 focus:bg-white focus:outline-hidden focus:ring-1 focus:ring-indigo-500 text-slate-800"
                  />
                </div>
              )}
              ...
              <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                {filteredFiles.map((file, idx) => {
                  const isChecked = selectedFilePaths.has(file.relativePath);
                  const isCopied = copiedKey === `parsed_${idx}`;
                  return (
                    <div
                      key={file.relativePath}
                      className="p-2 rounded-xl border border-slate-100 bg-slate-50 hover:bg-indigo-50/50 hover:border-indigo-200 transition-colors flex items-center justify-between text-xs"
                    >
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          ...
                          className="p-1 text-slate-500 hover:text-indigo-600 rounded hover:bg-slate-200/60 cursor-pointer"
                        >
                          <Edit3 className="w-3.5 h-3.5" />
                        </button>
                        <button
                          title="复制此文件代码"
                          onClick={() => handleCopyFileContent(file, `parsed_${idx}`)}
                          className="p-1 text-slate-500 hover:text-indigo-600 rounded hover:bg-slate-200/60 cursor-pointer"
                        >
                          {isCopied ? (
                        ...
                        <button
                          ...
                          className="p-1 text-slate-500 hover:text-emerald-600 rounded hover:bg-slate-200/60 cursor-pointer"
                        >
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 bg-white flex-1 overflow-auto flex flex-col space-y-2">
              <div className="text-xs text-slate-500 flex items-center justify-between">
                <span>可在下方直接编辑微调代码，保存后生效并同步至最终 Patch ZIP。</span>
                <span className="font-mono text-slate-400">行数: {editingContent.split('\n').length}</span>
              </div>
              <textarea
                value={editingContent}
                onChange={e => setEditingContent(e.target.value)}
                className="flex-1 w-full h-80 min-h-[300px] bg-slate-50 rounded-xl p-3 text-slate-800 font-mono text-xs border border-slate-300 focus:outline-hidden focus:ring-1 focus:ring-indigo-500 leading-relaxed resize-none selection:bg-indigo-100"
                spellCheck={false}
              />
            </div>
            ...
            <div className="px-5 py-3.5 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
              <span className="text-xs text-slate-500">
                {previewingFile.isSensitive ? '⚠️ 受保护敏感文件' : '常规源码文件'}
              </span>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg text-xs font-medium bg-indigo-600 hover:bg-indigo-700 text-white flex items-center gap-1.5 cursor-pointer shadow-xs"
                >
                  <Download className="w-3.5 h-3.5" />
                  下载此文件
                </button>
